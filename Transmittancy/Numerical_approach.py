...
v_min       = 1e3
v_max       = 120e3
v_ion_array = np.linspace(v_min,v_max,N)
vwpra_array = np.linspace(v_min,v_max,M)
beta        = 2.0
...
for n in range(0,len(v_ion_array)):
    ...
    print('(n,m) = ({},{}) out of ({},{})\n(v_ion,v_wpra) = ({:.1f}, {:.1f}) km/s'.format(n+1,m+1,N,M,v_ion_array[n]/1e3,vwpra_array[m]/1e3))
...
# CS = axs.contourf(XX,YY, count_collected.transpose()/N_points, levels=np.linspace(0,1.0,101),cmap='viridis')
# CS = axs.scatter(XX,YY,c=count_collected.transpose()/N_points,cmap='viridis',norm=mcolors.Normalize(vmin=0,vmax=1),s=30*XX*YY/((v_max-v_min)/1000)**2,marker='.',edgecolors='none')
...
axs.scatter(XX,YY,c='w',s=20*XX*YY/((axs.get_xlim()[1]-axs.get_xlim()[0]))**2,marker='.',edgecolors='none',zorder=4)
...
np.savetxt( myExBprobe.directory+'T_{}_n (M,N,N_points) = ({},{},{}), v = [{}, {}] kms {} (Moses).txt'.format(              ion_name,M,N,N_points,np.min(v_ion_array)/1e3,np.max(v_ion_array)/1e3,myExBprobe.memo),T_k,fmt='%s',delimiter=',')
plt.savefig(myExBprobe.directory+'T_{}_n (M,N,N_points) = ({},{},{}), v = [{}, {}] kms {} (α = {:.2f}°) (Moses).png'.format(ion_name,M,N,N_points,np.min(v_ion_array)/1e3,np.max(v_ion_array)/1e3,myExBprobe.memo,α_y),dpi=DPI, bbox_inches='tight')